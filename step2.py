...
ColorCenterX = configs['ColorCenterX']
ColorCenterY = configs['ColorCenterY']

if ColorCenterX == 'center':
    ColorCenterX = Img_width/2
if ColorCenterY == 'center':
    ColorCenterY = Img_height/2
...
#Generating Images
x = np.tile(np.expand_dims(np.arange(Img_width), 0), (Img_height, 1)) - ColorCenterX
y = (np.tile(np.expand_dims(np.arange(Img_height), 0).T, (1, Img_width)) - ColorCenterY)[::-1, :]
for idx, (angleframeshift, image_name) in enumerate(zip(np.linspace(0, 2*np.pi, Frames), image_names)):
    ...
    H = np.expand_dims(((HueSetting['Average'] + HueSetting['Deviation']*np.sin(HueSetting['Frequency']*angle + HueSetting['Shift']))%180).astype(np.uint8), 2)
